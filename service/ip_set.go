...
func (is *IpsetService) getIpsetService(config model.Config) *ipset.IPSet {
	...
	iptList, err := ipt.List("nat", "OUTPUT")
	for _, port := range config.CoveringPorts {
		...
		for _, rule := range iptList {
			if rule == fmt.Sprintf("-A OUTPUT -d %s/32 -p tcp -m set ! --match-set WhiteList src -m tcp --dport %d -j REDIRECT --to-ports %d", config.Bind, port, dstPort) {
				flag = true
		...
		if !flag {
			err := ipt.Append("nat", "OUTPUT", "-m", "set", "!", "--match-set", config.ListName, "src", "-d", config.Bind, "-p", "tcp", "--dport", strconv.FormatInt(int64(port), 10), "-j", "REDIRECT", "--to-port", strconv.FormatInt(int64(dstPort), 10))
			if err != nil {
